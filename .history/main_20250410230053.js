  ...
  document.getElementById("btn-add-picture").addEventListener("click", function () {
    ...
    input.addEventListener("change", function (e) {
      ...
      for (let i = 0; i < files.length; i++) {
        // Tìm ảnh chưa có (src trống hoặc là ảnh mặc định)
        while (
          imgIndex < imgElements.length &&
          imgElements[imgIndex].src &&
          !imgElements[imgIndex].src.includes("data:image") &&
          !imgElements[imgIndex].src.includes("default.jpg") // nếu bạn có ảnh mặc định
        ) {
          imgIndex++;
        }
        ...
        if (imgIndex < imgElements.length) {
          const reader = new FileReader();
          reader.onload = function (event) {
            imgElements[imgIndex].src = event.target.result;
          };
          reader.readAsDataURL(files[i]);
          imgIndex++;
        }
