  ...
  document.getElementById("btn-add-picture").addEventListener("click", function () {
    ...
    input.addEventListener("change", function (e) {
      ...
      let imgIndex = 0;
      // Tìm vị trí đầu tiên còn trống
      for (let i = 0; i < imgElements.length; i++) {
        if (!imgElements[i].src || imgElements[i].src.includes('placeholder') || imgElements[i].src === window.location.href) {
          imgIndex = i;
          break;
        }
      }
      ...
      for (let i = 0; i < imgElements.length && fileIndex < files.length; i++) {
        if (!imgElements[i].src || imgElements[i].src.includes('placeholder') || imgElements[i].src === window.location.href || imgElements[i].src.endsWith('/')) {
          const reader = new FileReader();
